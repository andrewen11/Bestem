# Bestem
Pe contul de Github se afla fisierele de configurare privind UI, respectiv (incercarea) de implementare a functionalitatilor cerute:

logare.html -> fisier html cu date despre pagina de logare //incercare relativ  esuata de implementare a functionalitatii de conectare la Google;
createaccount.html -> fisier html cu date despre pagina de sign up;
logare.css/ createaccount.css -> fisiere CSS specifice paginilor create;
logare.js -> incercarea de definire a unei functii pentru conectarea prin intermediul contului de Google;
buttonauth0.js -> Incercarea de stabilire a unei conexiuni intre butonul "Log me with Google" si auth0;
prezentare ppt legata de cerintele indeplinite pe parcursul celor 24 de ore;

*serverul WireGuard a fost transferat pe masina virtuala din Google Cloud din cauza incompatibilitatii de retea (ce aparea in lucrul cu aplicatia/ linia de comanda) | Se gaseste si aici, sub forma de fisiere de configurare

LINK PROIECT GOOGLE CLOUD / CLOUD STORAGE - buckets: https://console.cloud.google.com/storage/browser?_ga=2.62538028.1865443022.1702169867-329919836.1702139180&_gac=1.116646900.1702171637.CjwKCAiAvdCrBhBREiwAX6-6UvYwIYJEhW9FCg9B5YzIOzzU7Et0xvRfhRkfKBs73fWk8EyKoPCDDxoCneUQAvD_BwE&project=theta-arcana-407616&prefix=&forceOnBucketsSortingFiltering=true

Dezvoltat de Andrei Valentin-Bălălău (ACS - UPB) & Andrei-Cristian Prahoveanu (ETTI - UPB); 9 - 10 Decembrie 2023 | Biblioteca Centrală a Universității Politehnica din București
